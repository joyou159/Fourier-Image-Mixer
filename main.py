...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def load_ui_elements(self):
        self.ui_view_ports = [self.ui.original1, self.ui.original2, self.ui.original3, self.ui.original4]
        self.ui_combo_boxes = [self.ui.combo1, self.ui.combo2, self.ui.combo3, self.ui.combo4]
        self.ui_image_combo_boxes = [self.ui.combo11, self.ui.combo12, self.ui.combo13, self.ui.combo14]
        ...
        self.image_ports.extend([
            self.create_image_viewport(self.ui_view_ports[0], lambda event: self.browse_image(event, 0)),
            self.create_image_viewport(self.ui_view_ports[1], lambda event: self.browse_image(event, 1)),
            self.create_image_viewport(self.ui_view_ports[2], lambda event: self.browse_image(event, 2)),
            self.create_image_viewport(self.ui_view_ports[3], lambda event: self.browse_image(event, 3))
        ])
        ...
        for index, combo_box in enumerate(self.ui_combo_boxes):
            combo_box.currentIndexChanged.connect(self.handle_image_combo_boxes_selection)
    ...
    def handle_image_combo_boxes_selection(self):
        ...
        print(combo_ind)
        target_combo = self.ui_combo_boxes[self.ui_combo_boxes.index(sender_combo_box)]
